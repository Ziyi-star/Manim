...
class MeanValueTheoremForIntegrals(Scene):
    def construct(self):
        # Background grid
        grid = NumberPlane(
        ...
        # Define the curve
        f = lambda x: 0.1 * (x - 3)**3 + 1  # Third-degree curve
        ...
        rectangle = always_redraw(lambda: Rectangle(
            ...
            height=f(xi_tracker.get_value()),         # Height at f(ξ)
            color=BLUE,
            fill_opacity=0.5,
        ).move_to(ax.c2p((a + b) / 2, 0), DOWN))  # Align bottom edge at y=0 (x-axis)

        # Red dot at the top-right corner of the rectangle
        red_dot = always_redraw(lambda: Dot(
            ax.c2p(b, f(xi_tracker.get_value())),  # Top-right corner of the rectangle
            color=RED
        ))
        ...
        self.play(Create(rectangle), Create(xi_line), Create(red_dot))
