...
class XCosOneOverXZoom(ZoomedScene):
    def construct(self):
        # Background grid
        grid = NumberPlane(
        ...
        math_label = MathTex(r"f(x) = x \cdot \cos\left(\frac{1}{x}\right)").to_corner(UR)

        graph1 = axes.plot(func, x_range=[-1, -0.1], color=GREEN)
        graph2 = axes.plot(func, x_range=[-0.1, 0.1], color=GREEN, stroke_width=2)
        graph3 = axes.plot(func, x_range=[0.1, 1], color=GREEN)
