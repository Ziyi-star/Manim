...
class TaylorpolynomeT1Version3(ZoomedScene):
    def construct(self):
        # Background grid
        grid = NumberPlane(
        ...
        # Restglied
        def r_1(x):
            return exp_func(x)-(t_1(x))
        graph_r1 = axes.plot(r_1, color=GREEN, x_range=[-2.0, 1.9])
        label_r1 = MathTex("R_1(x) = e^x - (1 + x)", color=GREEN).next_to(label_t1, DOWN).scale(0.9)
        ...
        self.play(self.camera.frame.animate.scale(1/2).move_to(axes.c2p(0, 0)))  # Zoom in
        self.wait(5)
