...
class TaylorpolynomeT2(ZoomedScene):
    def construct(self):
        ...
        label_t2 = MathTex("T_2(x) = 1 + x + \\frac{x^2}{2}", color=BLUE).next_to(label_exp, DOWN, buff=0.1).scale(0.7).align_to(label_exp, LEFT)
        label_r2 = MathTex(r"R_2(x) = |f(x) - T_2(x)|", color=YELLOW).next_to(label_t2, DOWN).scale(0.7).align_to(label_exp, LEFT)
        label_r2_wert = always_redraw(
            lambda: MathTex(
                f"={(r_2(x_tracker.get_value())):.3f}"
            ).set_color(ORANGE)
            .next_to(label_r2, RIGHT,buff=0.3).scale(0.7)
        )
